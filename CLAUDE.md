...
app/
  ...
  (app)/
    ...
    oversikt/page.tsx          Dashboard: income-vs-expense difference, budget gauge, category breakdown
    oversikt/oversikt.module.css
    transaksjoner/page.tsx     Ledger: new-transaction row, RecurringPanel, activity table (search, CSV import/export)
    ...
    innsikt/page.tsx           Insights: MonthOverMonth, income-vs-expense net chart, fixed/variable/savings mix, category trends, subscriptions, Anomalies
...
components/
  ...
  MonthOverMonth.tsx  Comparison stats and movers vs. the previous period — a month against the month before, a year against the year before; on /innsikt
  Anomalies.tsx       Anomaly list for the selected month (incl. unbooked fixed expenses); last section on /innsikt
...
lib/
  ...
  period.ts, usePeriod.ts  URL period state (?p=...&w=...); period.ts also owns periodLabel, windowLabel/monthRangeLabel and the picker's window shape
  ...
  csv.ts              RFC 4180 CSV encoding (activity table export) and parsing (savings + transaction import), with delimiter sniffing
  importExpenses.ts   Transaction CSV import: header aliasing, row validation, the duplicate key and the kind guess for a category the file introduces
```
...
- `MonthColumns`' bars are deliberately **not** animated: every month key changes when the window shifts, so the entrance replayed in full on every press of the picker's arrows. Its hover band and tooltip still animate — pointer feedback, not data arriving. The hover band is drawn **only when `onSelect` is given**: it reads as "this column is pickable", so on the read-only charts (`/innsikt`'s net columns, `CategoryDrilldown`) a hover produces the tooltip and nothing else. The selected-month band is unconditional — state, not an affordance
...
- **The activity table's filter row is filters only, and the controls label themselves.** `Alle merkelapper` / `Alle kategorier` is both each select's placeholder and its reset value, so the uppercase `<label>` over every control is gone (an `aria-label` carries the name instead) — that caption stack was most of what made a row meant for skimming read as a form to fill in. `Nullstill filtre` is an `.icon-btn` that only exists while something is filtered, and the two CSV buttons moved to the card head, since importing and exporting are not ways of narrowing the list. The `Sortert på ...` caption left with them: the `DATO ⌄` column header already states the sort key and its direction
- **The search scope is a two-option select, and `Alle måneder` really means all of them.** It was a `Søk i hele perioden` checkbox that widened to the provider's *fetched* window — 24 months back plus whatever `ensureMonthCovered` had added, so a search that promised "hele perioden" quietly stopped at 27. Picking `Alle måneder (N)` now calls `ensureMonthCovered` on the first and last of `availableMonths` (which is all-time by design), so the window widens to the real span of the user's history before the filter runs; the option states `N` so the number is visible rather than implied. `RecurringPanel` still scopes to the selected month regardless — it answers "have this month's fixed expenses been booked", not "what does the table show"
- **`Importer CSV` reads a transaction file with a preview and no silent writes** (`lib/importExpenses.ts`, on top of `parseCsv` and `/sparing`'s `parseDateCell`/`parseAmountCell`). It needs a header row naming a date, a description and an amount; category and merkelapp are used when present, and a file exported from this same table reads straight back in. Three rules make it safe to press: the sign in the file is dropped from the amount (`expense.price` is unsigned — the category's kind carries direction) and only used to *guess* the kind of a category the file introduces, and only when the file contains both signs, since an all-positive statement says nothing about direction; rows matching one already in the ledger are counted and skipped, because `expense` cannot carry a unique constraint over columns the database cannot read; and rows whose category is blank are held out of the plan until a fallback category is picked. Everything — the row count, the date range, the duplicates, the categories about to be created and their guessed kinds, and every skipped line with its line number — is on screen before the confirm button writes anything
...
- Insight sections (`MonthOverMonth`, `Anomalies`) live on **`/innsikt`**, not `/oversikt`: both compare the selection with its recent past, which is an insight rather than an overview, and neither makes a fetch of its own. `Anomalies` sits **last** on the page — it is a list of things to go and check, not something to read the month by, and above the fold it pushed the month's own figures down. It takes the trailing 12-month window from `useLedgerHistory` — deliberately not `/innsikt`'s `useAnalysisWindow`, which is the same twelve months but clamped to the fetched range — because `detectAnomalies` treats what it is given as the complete history
- **`MonthOverMonth` compares a *period* with the period before it, not a month with the month before it.** `previousPeriod` (`lib/insights.ts`) shifts the selection back by its own length: one month lands on the month before, a year button's twelve months land on the same twelve a year earlier, four months on the previous four. `compareMonths` takes `MonthRef[]` and buckets by explicit month keys, so it is handed the **whole fetched ledger** rather than the 12-month history slice — the year before the selected one is outside that slice. `/innsikt` therefore carries its one `ensureMonthCovered` call, on the comparison's first month: unfetched months are indistinguishable from months where nothing happened, so without it picking an older year would report the previous year as a drop to zero. The card's title and its `mot ...` helper name what is being compared (`Sammenlignet med forrige måned` / `i fjor` / `forrige periode`), so the multi-month "velg én måned" hint is gone
- **`/oversikt` opens with the difference between income and expenses for the selected period** (`Inntekter mot utgifter`) — it answers "am I up or down this month / this year / all time" without reading a chart. It reuses the `summary` totals the gauge already computes, so nothing new is aggregated, and savings sit beside the difference rather than inside it (money set aside is not spent — the same reason the gauge's denominator excludes savings budgets). The period itself is the picker's job: a month, a whole year via the year buttons, or **`Alle`**, which selects every month that has data (`selectMonths` in `lib/usePeriod.ts`, driven by `availableMonths`, which is an all-time list)
- **The rest of `/oversikt` is the gauge and the breakdown, and nothing else.** The collapsible category list it used to carry was a second, read-only copy of `/budsjett`'s rows; the drill-down was only reachable from those rows, so it left with them. Both of the page's sections are conditional, so it also carries an explicit empty state — without one, a month with no budget and no spending renders blank, which reads as a failure
