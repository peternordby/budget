...
- **Component- or route-specific families live in a co-located `*.module.css`**, imported as `styles` and referenced as `styles["kebab-case-name"]` (not dot access, since the class names use hyphens):
  ...
  - `app/(app)/oversikt/oversikt.module.css` — the budget gauge (`.gauge-main-total` dims the "/ budget" half of the lead figure), expense-breakdown bar, and category-chart-row families, plus (single-consumer, relocated out of `globals.css`) `.insight-chip` (`.insight-good`/`.insight-warn`/`.insight-bad`), `.collapse-chevron` and `.category-value`. `.category-head` is gone — it was a private copy of `.card-head`. So are `.budget-popover*`, `.category-card.editing` and `.category-chart-row.is-editing`: budget editing moved to `/budsjett`, taking the popover, the pencil column and the z-index lifting that existed only to paint the popover over the next card.
  - `app/(app)/budsjett/budsjett.module.css` — the budget editor's row family (`.budget-list`, `.budget-header`, `.budget-row`, `.budget-total`, `.budget-name`, `.budget-dot`, `.budget-kind`, `.budget-input`, `.budget-actions`) and its full-bleed `.budget-track`/`.budget-bar`. It is deliberately a separate family from `/oversikt`'s `.category-chart-*` rather than a promotion of it: the two look alike but one is a read-only chart row and the other is a form row, and they diverge at the breakpoint (six columns collapsing to three, versus four to three).
  ...
  - `components/Avatar.module.css` — `.avatar` and its `.lg` size. Two sizes only: 26px for the nav chip, 56px for the profile header.
...
- `app/globals.css` defines `card-rise` (fade in + translate up), driving a page's top-level `.shell > section` entrance (staggered per section via `animation-delay`). It also drove the old `/oversikt` budget popover, which is gone.
...
- `860px`: the `/budsjett` row drops its header and folds six columns into three.
